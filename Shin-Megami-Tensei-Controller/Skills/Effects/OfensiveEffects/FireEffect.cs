...
public class FireEffect:Effect
{
    ...
    public FireEffect(Unit unitAttacking, int skillPower, View view) : base(unitAttacking)
    {
    ...
    public override void Apply(List<Unit> oponentUnits, TurnsController turnsController)
    {
        ...
        if (target != null)
        {
            menuController.ShowFireTarget(_unitAttacking, target);
            double damage = (Math.Sqrt(_unitAttacking.Magic * _skillPower));
        
            AffinitiesController affinitiesController = new AffinitiesController("Fire", damage, target, _unitAttacking, _view, turnsController);
