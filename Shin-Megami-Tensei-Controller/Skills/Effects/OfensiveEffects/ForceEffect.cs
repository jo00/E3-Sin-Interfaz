...
public class ForceEffect:Effect
{
    ...
    public ForceEffect(Unit unitAttacking, int skillPower, View view) : base(unitAttacking)
    {
    ...
    public override void Apply(List<Unit> oponentUnits, TurnsController turnsController)
    {
        ...
        if (target != null)
        {
            menuController.ShowForceTarget(_unitAttacking, target);
            double damage = (Math.Sqrt(_unitAttacking.Magic * _skillPower));
        
            AffinitiesController affinitiesController = new AffinitiesController("Force", damage, target, _unitAttacking, _view, turnsController);
